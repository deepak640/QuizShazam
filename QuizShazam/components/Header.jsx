...
export default function Header() {
  ...
  return (
    <>
      <Modal title={null} centered width={900} footer={null} open={chatOpen} onCancel={() => setChatOpen(false)}>
        <Chatbot />
      </Modal>

      <nav className={`sticky top-0 z-40 flex items-center justify-between px-6 py-3.5 transition-all duration-300 ${
        scrolled ? "bg-white/80 backdrop-blur-xl shadow-sm border-b border-slate-100" : "bg-white border-b border-slate-100"
      }`}>
        {/* Logo */}
        <button onClick={() => router.push("/")} className="flex items-center gap-2 group">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-700 to-indigo-500 flex items-center justify-center shadow-sm group-hover:shadow-md transition">
            <IoFlashOutline className="text-white" size={16} />
          </div>
          <span className="text-lg font-extrabold gradient-text">QuizShazam</span>
        </button>
        ...
        {user ? (
            ...
            {navActive && (
              <div className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl border border-slate-100 z-50 py-2 overflow-hidden">
                ...
                <button
                  ...
                  onClick={() => { setNavActive(false); setChatOpen(true); }}
                >
          ...
          <>
            <div className="hidden md:flex items-center gap-6">
              ...
              <Link href="/login" className="text-sm font-semibold text-slate-700 hover:text-violet-700 transition">Sign In</Link>
              <Link href="/register" className="text-sm font-semibold bg-gradient-to-r from-violet-700 to-indigo-500 text-white px-5 py-2 rounded-full hover:opacity-90 transition shadow-sm shadow-violet-100">
                Get Started
              </Link>
            </div>

            {/* Mobile menu */}
            <button className="md:hidden text-slate-700 hover:text-violet-700 transition" onClick={() => setNavActive((v) => !v)}>
              {navActive ? <IoCloseOutline size={24} /> : <IoMenuOutline size={24} />}
            </button>
            {navActive && (
              <div className="absolute top-full left-0 right-0 bg-white shadow-lg border-t border-slate-100 md:hidden z-50">
                <div className="flex flex-col px-6 py-4 gap-3">
                  <Link href="/" className="text-sm font-medium text-slate-700 py-2" onClick={() => setNavActive(false)}>Home</Link>
                  <Link href="/login" className="text-sm font-medium text-slate-700 py-2" onClick={() => setNavActive(false)}>Sign In</Link>
                  <Link href="/register" className="text-sm font-semibold text-violet-700 py-2" onClick={() => setNavActive(false)}>Create Account →</Link>
                </div>
              </div>
            )}
          </>
